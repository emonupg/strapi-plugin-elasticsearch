  ...
  searchCollection: async (ctx) => {
    try {
      ...
      if (!collectionName) {
        ...
        ctx.body = 'Collection name is required';
        return;
      }

      if (ctx.query.query) {
        const query = qs.parse(ctx.query.query);
        const resp = await esInterface.searchData(query, collectionName);
        if (resp?.hits?.hits) {
          const filteredData = resp.hits.hits.filter((dt) => dt._source !== null);
          const filteredMatches = filteredData.map((dt) => dt['_source']);
          ctx.body = filteredMatches;
      ...
      ctx.response.status = 500;
      ctx.body = 'An error was encountered while processing the search request.';
      console.log('An error was encountered while processing the collection search request.');
      console.log(err);
    }
  },
};
