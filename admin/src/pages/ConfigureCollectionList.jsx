...
import {
  apiGetContentConfig,
  apiRequestCollectionIndexing,
  apiImportContentConfig,
  apiExportContentConfig,
  apiRebuildCollectionIndex,
} from '../utils/apiUrls';
import { Alert } from '@strapi/design-system';
import { Table, Thead, Tbody, Tr, Td, Th } from '@strapi/design-system';
import { Typography } from '@strapi/design-system';
import { Pencil, Server, ArrowClockwise } from '@strapi/icons';
...
const Configure = () => {
  ...
  const [rebuildingCollection, setRebuildingCollection] = useState(null);
  ...
  const loadContentConfig = () => {
    return get(apiGetContentConfig).then((resp) => resp.data);
  };
    ...
    return get(apiRequestCollectionIndexing(collectionName))
      .then(() => {
        showMessage({
          variant: 'success',
          title: 'Success',
          text: `Collection ${collectionName} scheduled for indexing.`,
        });
      })
      .catch((err) => {
        showMessage({
          variant: 'warning',
          title: 'Error',
          text: 'Scheduling collection indexing failed. An error was encountered.',
        });
        console.log(err);
      })
      .finally(() => {
  ...
  const rebuildCollectionIndex = () => {
    if (!rebuildingCollection) return;

    setIsInProgress(true);
    setShowRebuildConfirm(false);

    post(apiRebuildCollectionIndex(rebuildingCollection))
      .then((resp) => {
        ...
        if (result.success) {
          showMessage({
            ...
            text: `Index for ${rebuildingCollection} rebuilt successfully.`,
          });
        } else {
          showMessage({
            variant: 'warning',
            title: 'Rebuild completed with issues',
            text: result.error || 'Some validation checks failed.',
          });
        }
      })
      .catch((err) => {
        showMessage({
          ...
          text: err.message || 'An error occurred during rebuild.',
        });
        console.log(err);
      })
      .finally(() => {
        window.scrollTo({ top: 0, behavior: 'smooth' });
        setIsInProgress(false);
        setRebuildingCollection(null);
      });
  };

  const handleRebuildClick = (collectionName) => {
    setRebuildingCollection(collectionName);
    ...
    exportContentConfig()
      .then(() => {
        showMessage({
          variant: 'success',
          title: 'Success',
          text: 'Collections configuration exported.',
        });
      })
      .catch((err) => {
        showMessage({
          variant: 'warning',
          title: 'Error',
          text: 'Exporting collections configuration failed. An error was encountered.',
        });
        console.log(err);
      })
  ...
  const performImport = () => {
    if (!importJson || !isEnteredJsonValid) return;

    setIsInProgress(true);
    const conf = JSON.parse(importJson);
    importContentConfig(conf)
      .then(() => {
        showMessage({
          variant: 'success',
          title: 'Success',
          text: 'Collections configuration imported.',
        });
        setImportJson(null);
        ...
        for (let r = 0; r < Object.keys(resp).length; r++) {
          ...
          for (let k = 0; k < Object.keys(resp[collectionName]).length; k++) {
            ...
            for (let s = 0; s < Object.keys(attribs).length; s++) {
              ...
              else if (attr.index && !item.indexed.includes(attrName)) item.indexed.push(attrName);
            }
          }
      ...
      .catch((err) => {
        showMessage({
          variant: 'warning',
          title: 'Error',
          text: 'Importing collections configuration failed. An error was encountered.',
        });
        console.log(err);
      })
      .finally(() => setIsInProgress(false));
    ...
    loadContentConfig()
      ...
      .catch((err) => {
        showMessage({
          variant: 'warning',
          title: 'Error',
          text: 'An error was encountered while fetching the configuration.',
        });
        console.log(err);
      })
      .finally(() => {
  ...
  return (
    <Page.Main>
                        ...
                        <Td>
                          <Flex gap={2}>
                            <TooltipIconButton
                              ...
                              onClick={() =>
                                navigate(
                                  `/plugins/${pluginId}/configure-collections/${collection.collectionName}`
                                )
                              }
                            ...
                            <TooltipIconButton
                              withTooltip={false}
                              onClick={() => handleRebuildClick(collection.collectionName)}
                              label="Rebuild index for this collection"
                              noBorder
                            >
                              <ArrowClockwise />
                            </TooltipIconButton>
      ...
      {showRebuildConfirm && (
        <Modal.Root open={showRebuildConfirm} onOpenChange={setShowRebuildConfirm}>
          <Modal.Content>
            <Modal.Header>
              <Modal.Title>Rebuild Collection Index</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <Typography>
                Are you sure you want to rebuild the index for {rebuildingCollection}? This will
                recreate the index and re-index all documents.
              </Typography>
            </Modal.Body>
            <Modal.Footer>
              <Button onClick={() => setShowRebuildConfirm(false)} variant="tertiary">
                Cancel
              </Button>
              <Button onClick={rebuildCollectionIndex} variant="danger" loading={isInProgress}>
                Rebuild
              </Button>
            </Modal.Footer>
          </Modal.Content>
